...
export class CombatScene extends Phaser.Scene {
    ...
    async _buildCompanions() {
        ...
        const { getCompanion } = await import('../data/companions/index.js');
    ...
    _buildMonsterArea() {
        ...
        this.monsters.forEach((monster, i) => {
            ...
            // Element badge di bawah nama monster
            if (monster.element) {
                this.add.text(x, y + 114, `${_elementIcon(monster.element)} ${monster.element}`, {
                    fontFamily: 'monospace', fontSize: '9px', color: '#445566',
                }).setOrigin(0.5);
            }

            const statusTxt = this.add.text(x, y + 128, '', {
            ...
            const targetArrow = this.add.text(x, y - 120, '▼ TARGET', {
                fontFamily: 'monospace', fontSize: '12px',
                color: '#ffcc44', fontStyle: 'bold',
                stroke: '#000000', strokeThickness: 3,
            }).setOrigin(0.5).setVisible(i === 0).setDepth(10);
    ...
    _selectTarget(index) {
        ...
        this.monsterTargetRings.forEach((ring, i) => {
            ...
            ring.setStrokeStyle(selected ? 3 : 0, 0xffcc44);
            ring.targetGlow?.setVisible(selected);
            this.monsterTargetLabels?.[i]?.setVisible(selected);

            const spr = this.monsterSprites?.[i];
            if (!spr) return;
            this.tweens.add({
                targets: spr,
                y:     selected ? this.nodePositions[i].y - 6 : this.nodePositions[i].y,
    ...
    _buildPlayerArea() {
        ...
        this.paperDoll = new PaperDollDisplay(this, px, py - 95, this.player, {
            scale: 2.4, depth: 2, interactive: false,
        });
    ...
    _buildTooltip() {
        this.tooltipBg = this.add.rectangle(0, 0, 230, 175, 0x080812)
            .setStrokeStyle(1, 0x223344).setDepth(50).setVisible(false);
    ...
    _showTooltip(card, cardX, cardY) {
        ...
        const elemStr = card.element ? `Elemen: ${card.element}` : '';
        ...
        const lines = [
            ...
            { text: elemStr || card.flavorText || '', color: '#334455', size: '9px', dy: 130 },
    ...
    _createCardObject(card, x, y, handIndex, inQueue = false) {
        ...
        const typeColors = {
            attack: 0xcc4444, defense: 0x2244cc,
            magic:  0x8833cc, support: 0x33aa44,
        };
        ...
        if (!inQueue && affordable) {
            ...
            bg.on('pointerover', () => {
                ...
                if (card.damage) {
                    const ring = this.monsterTargetRings?.[this.selectedTarget];
                    if (ring) {
                        this.tweens.add({ targets: ring, scale: 1.18, duration: 120, yoyo: true, repeat: 1 });
                    }
                }
            });
            ...
            switch (evt.type) {
                ...
                case 'damage': {
                    ...
                    if (pos) {
                        ...
                        // ── FIX: element reaction label ──────────
                        if (evt.elementReaction && evt.elementReaction !== 'neutral') {
                            DamageNumber.showReaction?.(this, pos.x, pos.y - 62, evt.elementReaction);
                        }
                    }
                ...
                case 'phase_change': {
                    ...
                    if (pos) DamageNumber.show(this, pos.x, pos.y - 40, evt.phaseIndex || 2, 'phase');
                    break;
                }
                ...
                case 'apply_status': {
                    const pos = evt.target === 'player'
                        ? this._getPlayerPos()
                        : this._getTargetPos(evt.target);
                    if (pos) DamageNumber.showStatus?.(this, pos.x, pos.y - 20, evt.status, evt.value);
                    break;
                }
                ...
                case 'pet_revive': {
                    const px = this._getPlayerPos();
                    DamageNumber.show(this, px.x, px.y - 80, 0, 'heal');
                    this._showPhaseAnnouncement('🔥 Feniks Bangkit!');
                    break;
                }

                case 'companion_damage': {
                    const pos = this._getTargetPos(evt.target);
                    if (pos) DamageNumber.show(this, pos.x, pos.y - 30, evt.amount, evt.damageType || 'physical');
                    break;
                }

                case 'companion_heal': {
                    const px = this._getPlayerPos();
                    DamageNumber.show(this, px.x - 30, px.y - 40, evt.amount, 'heal');
                    break;
                }

                case 'companion_ulti': {
                    this._showPhaseAnnouncement(`✦ ${evt.ultiName}!`);
                    break;
                }
            }
        }
    ...
    _showPhaseAnnouncement(text) {
        ...
        this.tweens.add({
            targets: [overlay, txt], alpha: { from: 0, to: 1 }, duration: 300,
            onComplete: () => {
                this.time.delayedCall(1500, () => {
                    this.tweens.add({
                        targets: [overlay, txt], alpha: 0, duration: 400,
    ...
    _refreshUI() {
        ...
        this.monsters.forEach((monster, i) => {
            ...
            this.monsterHpBars[i].setScale(Math.max(0, mHpPct), 1);
                ...
                action: () => {
                    import('../storage/SaveSystem.js').then(({ SaveSystem }) => {
                        SaveSystem.manualSave({
                            ...
                            mapData: this.mapData, currentNodeId: 'start',
                        });
